...
class DataBaseAdapter:
    ...
    def get_compare_aggregate_response_times_for_test_ids(self, test_id_1, test_id_2):
        ...
        statement = select([
            ...
            (((a1.c.average-a2.c.average)/a2.c.average)*100).label('avg_diff_percent'),
            a1.c.median.label('median_1'),
            a2.c.median.label('median_2'),
            (a1.c.median-a2.c.median).label('med_diff'),
            (((a1.c.median-a2.c.median)/a2.c.median)*100).label('med_diff_percent')
    ...
    def get_compare_tests_cpu(self, test_id, num_of_prev_tests):
        ...
        statement = select([
                            s.c.display_name,
                            sqlalchemy.func.regexp_replace( s.c.server_name,'\\.','_').label('server_name'),
                            s.c.CPU_LOAD]).where(s.c.rank<=num_of_tests+1)
        return self.execute_statement(statement, True)
