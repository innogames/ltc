...
class DataBaseAdapter:
    ...
    def get_aggregate_table_for_test_id(self, test_id):
        statement = select([
            aggregate.c.URL,
            aggregate.c.average,
            aggregate.c.median,
    ...
    def getCompareRTOTDataForTestIds(self, test_id_1, test_id_2):
        ta1 = select([
            ...
            func.row_number().over(
                order_by=tests_overall_data.c.timestamp).label('rown'),
    ...
    def get_compare_aggregate_response_times_for_test_ids(self, test_id_1,
                                                          test_id_2):

        agg1 = select(
            [aggregate.c.URL, aggregate.c.average,
             aggregate.c.median]).where(aggregate.c.test_id == test_id_1)
        ...
        a2 = agg2.alias('a2')
        statement = select([
            a1.c.URL.label('URL'), a1.c.average.label('average_1'),
            a2.c.average.label('average_2'),
            (a1.c.average - a2.c.average).label('avg_diff'), (
                ((a1.c.average - a2.c.average) /
                 a2.c.average) * 100).label('avg_diff_percent'),
            a1.c.median.label('median_1'), a2.c.median.label('median_2'),
            (a1.c.median - a2.c.median).label('med_diff'), (
                ((a1.c.median - a2.c.median) /
                 a2.c.median) * 100).label('med_diff_percent')
    ...
    def getCompareAggregateDataForTestIds(self, test_id_1, test_id_2):
        ...
        agg1 = select([
            aggregate.c.URL, aggregate.c.average, aggregate.c.median,
            aggregate.c['75_percentile'], aggregate.c['90_percentile'],
            aggregate.c['99_percentile'], aggregate.c.maximum,
            aggregate.c.minimum, aggregate.c.count, aggregate.c.errors,
            aggregate.c.weight
    ...
    def get_compare_response_times_for_test_ids(self, test_id_1, test_id_2,
                                                mode):
    ...
    def getReleasesList(self):
        statement = select([tests.c.id, tests.c.display_name])
        return self.execute_statement(statement, False)

    def get_tests_list(self, project_name):
        ''' Return test`s list for project '''
        statement = select([tests.c.id, tests.c.display_name
                            ]).where(tests.c.project == project_name).order_by(
                                desc(tests.c.start_time))
        return self.execute_statement(statement, False)

    def get_max_test_id_for_project_name(self, project_name):
        log.debug("Get max test id for project_name: " + project_name)
        statement = select(
            [func.max(tests.c.id)]).where(tests.c.project == project_name)
        return self.execute_statement(statement, False)

    def get_min_test_id_for_project_name(self, project_name):
        log.debug("Get min test id for project_name: " + project_name)
        statement = select(
            [func.min(tests.c.id)]).where(tests.c.project == project_name)
    ...
    def get_compare_avg_cpu_load_data_for_test_ids(self, test_id_1, test_id_2):
        ...
        qr = select([
            qt.c.server_name, func.sum(qt.c.CPU_LOAD_1).label(test_1_name),
    ...
    def get_projects_list(self):
        statement = select([distinct(tests.c.project)])
        return self.execute_statement(statement, False)

    def get_last_test(self, project_name):
        stmt = select([func.max(tests.c.start_time).label('max_start_time')
                       ]).where(tests.c.project == project_name)
        s = stmt.alias('s')
        statement = select(
            [tests]).where(tests.c.project == project_name).where(
                tests.c.start_time == s.c.max_start_time)
        return self.execute_statement(statement, False)

    def get_test_info(self, project_name, build_number):
        statement = select(
            [tests]).where(tests.c.project == project_name).where(
                tests.c.build_number == build_number)
        return self.execute_statement(statement, False)

    def get_test_info_from_id(self, test_id):
        statement = select([tests]).where(tests.c.id == test_id)
        return self.execute_statement(statement, False)
    ...
    def get_rtot_for_url(self, test_id, url):
        statement = select([
            tests_url_data.c.timestamp,
            tests_url_data.c.avg.label('average'),
            tests_url_data.c.median.label('median'),
            tests_url_data.c.errors.label('errors')
    ...
    def get_errors_percent_for_test_id(self, test_id):
        statement = select([
            func.sum(aggregate.c.count * aggregate.c.errors / 100) * 100 /
            func.sum(aggregate.c.count)
        ]).where(aggregate.c.test_id == test_id)
        return self.execute_statement(statement, False)
    ...
    def get_response_codes_for_test_id(self, test_id):
        statement = select([
            100]) \
            .where(aggregate.c.test_id == test_id)
        return self.execute_statement(statement, False)

    def get_top_requests_by_avg(self, test_id, top_number):
        statement = select([
            aggregate.c.URL,
            aggregate.c.average
        ])\
            .where(aggregate.c.test_id == test_id)\
            .order_by(desc(aggregate.c.average))\
            .limit(top_number)
        return self.execute_statement(statement, True)

    def get_top_requests_by_errors(self, test_id, top_number):
        statement = select([
            aggregate.c.URL,
            aggregate.c.errors
        ]) \
            .where(aggregate.c.test_id == test_id) \
            .order_by(desc(aggregate.c.errors)) \
            .limit(top_number)
        return self.execute_statement(statement, True)
    ...
    def get_monitoring_data(self, test_id, server_name, metric):
        ...
        if metric == "CPU_all":
            ...
            stmt2 = select([
                (func.date_trunc('minute', tests_overall_data.c.timestamp) -
                 mts2.c.min_ts).label('timestamp'), literal(0).label(metric),
                (tests_overall_data.c.count / 60).label('rps')
            ]).where(tests_overall_data.c.test_id == test_id).order_by(
                asc(tests_overall_data.c.timestamp))
            ...
            statement = select([
                qt.c.timestamp, func.sum(qt.c[metric]).label(metric),
    ...
    def get_metric_max_value(self, test_id, server_name, metric):
        if "CPU" in metric:
            statement = select([literal("100").label('max_value')])
        elif "RPS" in metric:
            statement = select([
                (func.max(tests_overall_data.c.count) / 60).label('max_value')
            ]).where(tests_overall_data.c.test_id == test_id)
        else:
            statement = select([
                func.max(tests_monitoring_data.c[metric]).label('max_value')
            ]).where(tests_monitoring_data.c.test_id == test_id).where(
                tests_monitoring_data.c.server_name == server_name)
        return self.execute_statement(statement, True)
    ...
    def get_prev_test_id(self, test_id, offset):
        ...
        stmt = select([
            func.row_number().
                over(order_by=desc(tests.c.start_time)).
                label('rown'),
            tests.c.id
        ...
        s = stmt.alias('s')
        statement = select([s.c.id]).where(s.c.rown == offset)
   ...
   .where(tests.c.id == aggregate.c.test_id) \
        ...
        statement = select([#s.c.start_time,
            ...
            s.c.display_name,
            s.c.average,
            s.c.median]).limit(num_of_tests+1)
        return self.execute_statement(statement, True)
    ...
    def get_compare_tests_cpu(self, test_id, num_of_prev_tests):
        ...
        s = stmt.alias('s')
        statement = select([
            s.c.display_name, sqlalchemy.func.regexp_replace(
                s.c.server_name, '\\.', '_').label('server_name'), s.c.CPU_LOAD
    ...
    def get_bounded_overall_compare_data_for_project_name(
        ...
        log.debug("Get bounded overall data for project_name: " + project_name
                  + ", test_id_min: " + str(test_id_min) + "; test_id_max: " +
                  str(test_id_max) + ";")
    ...
    def get_servers_for_test_id(self, test_id):
    ...
    def get_monitoring_metrics(self):
        statement = select([tests_monitoring_data]).columns.keys()
