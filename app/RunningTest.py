...
class RunningTest:
    ...
    def update_data_frame(self):
        log.debug("CALL update_data_frame")
        log.debug(self.jmeter_results_file + " file size:" + str(os.path.getsize(self.jmeter_results_file)))
        log.debug("Start line for parsing:" + str(self.start_line));
        num_lines = sum(1 for line in open(self.jmeter_results_file))
        log.debug("Num lines in the file:" + str(num_lines));
        if self.start_line < num_lines - 10:
            read_lines = num_lines - self.start_line - 10;
            log.debug("read_lines in the file:" + str(read_lines));
        #if self.file_size < os.path.getsize(self.jmeter_results_file):
            #self.file_size = os.path.getsize(self.jmeter_results_file)
            log.debug("Adding a new data from file. Start line: " + str(self.start_line));
            df = pd.read_csv(self.jmeter_results_file, index_col=0, low_memory=False,
                             skiprows=self.start_line, nrows=read_lines)
            ...
            df1 = pd.concat([self.response_codes_frame,add_df]).groupby('response_code')['count'].sum().reset_index()
            self.response_codes_frame = df1
            print add_df
            #create aggregate table
            group_by_url = df.groupby('URL') # group date by URLs
            add_aggregate_data = group_by_url.aggregate({'average':np.mean}).round(1)
            ...
            add_aggregate_data['errors'] = df[(df.success == False)].groupby('URL')['success'].count()
            add_aggregate_data = add_aggregate_data.fillna(0)
            add_aggregate_data = add_aggregate_data.reset_index()
            add_aggregate_data.columns = ['URL', 'average','maximum','minimum','count','errors']
            #???
            df1 = pd.concat([self.aggregate_frame,add_aggregate_data]).groupby('URL')['average'].mean().reset_index()
            df2 = pd.concat([self.aggregate_frame,add_aggregate_data]).groupby('URL')['count','errors'].sum().reset_index()
            df3 = pd.concat([self.aggregate_frame,add_aggregate_data]).groupby('URL')['maximum'].max().reset_index()
            df4 = pd.concat([self.aggregate_frame,add_aggregate_data]).groupby('URL')['minimum'].min().reset_index()
            ...
            gr_by_minute = df.groupby(pd.TimeGrouper(freq='1Min')) # group data by minute
            ...
            add_df2['errors_count'] = df[(df.success == False)].groupby(pd.TimeGrouper(freq='1Min'))['success'].count()
            ...
            add_df2.columns = ['time', 'average','median','count','errors_count']
            df1 = pd.concat([self.data_frame,add_df2]).groupby('time')['average','median'].mean().reset_index()
            df2 = pd.concat([self.data_frame,add_df2]).groupby('time')['count','errors_count'].sum().reset_index()
            ...
            log.debug(".jtl file was not changed")

    def update_successful_requests_percentage(self):
        df = self.data_frame
    ...
    def update_last_minute_avg_rps(self):
        df = self.data_frame
