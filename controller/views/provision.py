...
from controller.models import Proxy, TestRunning, LoadGeneratorServer, JMeterTestPlanParameter, ScriptParameter, LoadGenerator, JmeterInstance
...
def update_load_generators_info():
    ...
    hosts = query(
        ...
        function='loadgenerator',# hostname=Any('loadtest10-qa','loadtest09-qa')
        )
    for host in hosts:
        t = threading.Thread(
    ...
    for generator in list(LoadGenerator.objects.values()):
        ...
        if not hostname in str(load_generators_info):
            logger.debug("Remove loadgenerator from database: {}".format(hostname))
            LoadGenerator.objects.filter(hostname=hostname).delete()


def get_load_generators_data(request):
    load_generators_data = list(LoadGenerator.objects.annotate(jmeter_instances_count=Count('jmeterinstance__id')).values('hostname',
        'num_cpu',
        'memory',
        'memory_free',
        'la_1',
...
def prepare_load_generators(project_name, jmeter_dir, threads_num,
    ...
    for generator in load_generators_info:
        ...
        if required_memory_total < memory_free and status == 'success':
            logger.info("Found a single load generator: {}; required_memory_total: {}; memory_free: {}".format(hostname,
        required_memory_total, memory_free))
            # If first server is mathed ok
            matched_load_generators.append({'hostname': hostname,
                                            'target_amount_jri': target_amount_jri})
            overall_hosts_amount_jri = target_amount_jri
            ready = True
            break
            
    if ready == False:
        logger.info("Did not a single load generator. Trying to find a combination.")
        t_hosts = {}
        ...
        for generator in load_generators_info:
            ...
            if p > 10:
                p = 10
            t_hosts[hostname] = math.ceil(memory_free/(required_memory_for_jri * 1.3))
        t_sorted_hosts = sorted(t_hosts, key=t_hosts.get, reverse=True)
        
        for h in t_sorted_hosts:
            possible_jris_on_host = t_hosts[h]
            logger.debug("h: {}; possible_jris_on_host: {};".format(h, possible_jris_on_host))
            ...
            matched_load_generators.append({'hostname': h,
                                            'possible_jris_on_host': int(possible_jris_on_host)})
            overall_hosts_amount_jri += possible_jris_on_host
            logger.debug("overall_hosts_amount_jri: {};".format(overall_hosts_amount_jri))
    ...
    if ready == False and overall_hosts_amount_jri < target_amount_jri:
        ...
        #                    mb_per_thread, duration)
    logger.debug("matched_load_generators: {};".format(str(matched_load_generators)))
    ...
    response = {"remote_hosts_string": final_str,
...
def stop_jmeter_instance(jmeter_instance):
    ...
    if check_pattern.match(jmeter_dir) is not None:
        logger.info('Removing remote Jmeter instance directory: {} from remote host: {}'.format(
        jmeter_dir, hostname))
